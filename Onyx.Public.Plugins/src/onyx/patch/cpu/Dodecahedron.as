...
package onyx.patch.cpu {
	...
	final public class Dodecahedron extends PluginPatchCPU {
		...
		private var _matbuf:Matrix;
		private var gl:Render3D;
		...
		override public function initialize(context:IDisplayContextCPU, channel:IChannelCPU, path:IFileReference, content:Object):PluginStatus {
			...
			_light = new Light(100,0.5,0.25);
			_screen = new DisplaySurface(context.width, context.height, true, 0x00);
			_matbuf = new Matrix(1, 0, 0, 1, context.width / 2, context.height / 2);
			gl = new Render3D(300,1);
			ss = new StructureSynth();
			
			camera = new Vector3D(0, 0, -50);
			_depth = new BitmapData(context.width, context.height, false, 0);
			_ssao  = new BitmapData(context.width, context.height, false, 0);
			_mask  = new BitmapData(context.width, context.height, false, 0);
